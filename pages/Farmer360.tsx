    ...
    return (
        <div className="space-y-4 max-w-[1920px] mx-auto pb-6">
            ...
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 text-xs text-aqua-400 font-medium">
                ...
                <div className="flex flex-wrap justify-between items-end gap-4">
                    ...
                    {view === 'list' && (
                        <div className="flex gap-3">
                            <div className="bg-aqua-900/50 border border-aqua-700/50 rounded-lg px-3 py-1.5">
                                <p className="text-[10px] text-gray-400 uppercase font-semibold">Total Farmers</p>
                                <p className="text-lg font-bold text-white">{totalFarmersCount}</p>
                            </div>
                            <div className="bg-aqua-900/50 border border-aqua-700/50 rounded-lg px-3 py-1.5">
                                <p className="text-[10px] text-gray-400 uppercase font-semibold">Total Acres</p>
                                <p className="text-lg font-bold text-aqua-400">{totalAcres}</p>
                            </div>
                             <div className="bg-aqua-900/50 border border-aqua-700/50 rounded-lg px-3 py-1.5 hidden sm:block">
            ...
            {(view === 'list' || view === 'approvals') && (
                <div className="space-y-4">
                    {/* Control Bar */}
                    <div className="flex flex-col md:flex-row gap-3 justify-between items-center bg-aqua-800/30 p-2 rounded-xl border border-aqua-800 backdrop-blur-sm">
                        <div className="relative w-full md:w-80">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-aqua-400" size={14} />
                            <input 
                                type="text" 
                                placeholder="Search by Name, ID, or Location..." 
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="w-full bg-aqua-900 border border-aqua-700 rounded-lg pl-9 pr-4 py-2 text-xs text-white focus:outline-none focus:border-aqua-500 focus:ring-1 focus:ring-aqua-500 placeholder:text-gray-500 transition-all"
                            />
                        </div>
                        
                        <div className="flex items-center gap-2 w-full md:w-auto">
                             <span className="text-xs text-gray-400 hidden md:inline">Filter by Location:</span>
                             <div className="relative flex-1 md:w-40">
                                <MapPin size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-aqua-400" />
                                <select 
                                    value={filterRegion} 
                                    onChange={(e) => setFilterRegion(e.target.value)}
                                    className="w-full appearance-none bg-aqua-900 border border-aqua-700 rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-aqua-500 cursor-pointer"
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4">
                        {filteredFarmers.length === 0 ? (
                            ...
                            <div key={farmer.id} className="group relative bg-aqua-800/40 border border-aqua-700/60 p-4 rounded-xl hover:bg-aqua-800/80 hover:border-aqua-500/50 hover:shadow-xl hover:shadow-aqua-500/10 transition-all duration-300 flex flex-col">
                                {/* Card Header */}
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <div className="relative">
                                            <div className="h-12 w-12 rounded-full bg-cover bg-center border-2 border-aqua-700 group-hover:border-aqua-500 transition-colors" style={{ backgroundImage: `url(${farmer.avatar})` }}></div>
                                            <span className={`absolute -bottom-1 -right-1 h-3 w-3 rounded-full border-2 border-aqua-900 ${farmer.status === 'Active' ? 'bg-green-500' : farmer.status === 'Inactive' ? 'bg-gray-500' : 'bg-yellow-500'}`}></span>
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-white text-base leading-tight group-hover:text-aqua-400 transition-colors">{farmer.name}</h3>
                                            <p className="text-[10px] text-gray-400 flex items-center gap-1 mt-1"><MapPin size={10} className="text-aqua-500"/> {farmer.location}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-3">
                                    <div className="bg-aqua-900/60 p-2 rounded-lg border border-aqua-800">
                                        ...
                                        <p className="text-white font-bold text-sm">{farmer.acres}</p>
                                    </div>
                                    <div className="bg-aqua-900/60 p-2 rounded-lg border border-aqua-800">
                                        <div className="flex items-center gap-1.5 mb-1 text-gray-400 text-[10px] uppercase font-bold tracking-wider">
                                            <Droplets size={10} /> Ponds
                                        </div>
                                        <p className="text-white font-bold text-sm">{farmer.ponds}</p>
                                    </div>
                                </div>

                                {/* Technician Info */}
                                <div className="bg-aqua-900/30 rounded-lg p-2 mb-3 flex items-center gap-2 border border-aqua-800/50">
                                     <div className="bg-aqua-800 p-1.5 rounded-full">
                                        <User size={12} className="text-aqua-400" />
                                     </div>
                                     <div className="overflow-hidden">
                                        <p className="text-[10px] text-gray-500 uppercase font-semibold">Assigned Technician</p>
                                        <p className="text-xs text-gray-200 truncate">{farmer.technicianName}</p>
                                     </div>
                                </div>

                                {/* Actions */}
                                <div className="mt-auto pt-3 border-t border-aqua-800/50 flex gap-2">
                                    {view === 'list' ? (
                                        <button 
                                            onClick={() => handleViewDetails(farmer)}
                                            className="w-full py-2 bg-aqua-500 hover:bg-aqua-400 text-aqua-950 font-bold rounded-lg transition-all shadow-lg shadow-aqua-500/10 text-xs"
                                        >
                                            View Profile
                                        </button>
                                    ) : (
                                        <>
                                            ...
                                            <button className="flex-1 py-2 bg-red-900/50 hover:bg-red-900 text-red-400 border border-red-800 rounded-lg transition-colors text-xs font-bold flex items-center justify-center gap-1">
            ...
            {view === 'details' && selectedFarmer && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <button onClick={() => setView('list')} className="group flex items-center gap-2 text-gray-400 hover:text-aqua-400 transition-colors mb-2">
                        <div className="p-1 bg-aqua-900 rounded-full group-hover:bg-aqua-800"><ArrowLeft size={14} /></div>
                        <span className="text-xs font-medium">Back to Directory</span>
                    </button>

                    {/* Large Profile Header */}
                    <div className="relative overflow-hidden bg-gradient-to-br from-aqua-800 to-aqua-900 border border-aqua-700 rounded-2xl p-5 shadow-2xl">
                        <div className="relative z-10 flex flex-col md:flex-row gap-6 items-center md:items-start">
                            <div className="h-32 w-32 rounded-full bg-cover bg-center ring-4 ring-aqua-900/50 shadow-xl" style={{ backgroundImage: `url(${selectedFarmer.avatar})` }}></div>
                            
                            <div className="flex-1 text-center md:text-left space-y-1.5">
                                <div className="flex flex-col md:flex-row items-center gap-3">
                                    <h2 className="text-3xl font-black text-white tracking-tight">{selectedFarmer.name}</h2>
                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${selectedFarmer.status === 'Active' ? 'bg-green-500 text-green-950' : 'bg-gray-600 text-gray-200'}`}>
                                        {selectedFarmer.status}
                                    </span>
                                </div>
                                <p className="text-aqua-200 text-sm font-medium flex items-center justify-center md:justify-start gap-1">
                                    <MapPin size={14} /> {selectedFarmer.location}
                                </p>
                                <div className="flex flex-wrap gap-2 mt-2 justify-center md:justify-start">
                                    <span className="flex items-center gap-1.5 text-xs text-white bg-white/10 px-3 py-1.5 rounded-full backdrop-blur-sm">
                                        <LandPlot size={12} className="text-aqua-300"/> {selectedFarmer.acres} Acres
                                    </span>
                                    <span className="flex items-center gap-1.5 text-xs text-white bg-white/10 px-3 py-1.5 rounded-full backdrop-blur-sm">
                                        <Droplets size={12} className="text-aqua-300"/> {selectedFarmer.ponds} Ponds
                                    </span>
                                </div>
                            </div>

                             <div className="bg-black/30 p-4 rounded-xl border border-white/10 backdrop-blur-sm w-full md:w-auto min-w-[240px]">
                                <h4 className="text-[10px] font-bold text-aqua-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                                    <User size={12}/> Assigned Technician
                                </h4>
                                <div className="flex items-center gap-3">
                                    <div className="h-8 w-8 rounded-full bg-gray-700 flex items-center justify-center text-white font-bold text-xs">MT</div>
                                    <div>
                                        <p className="text-white font-bold text-sm">{selectedFarmer.technicianName}</p>
                                        <p className="text-xs text-gray-300">{selectedFarmer.technicianPhone}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                     ...
                     <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            <h3 className="text-xl font-bold text-white flex items-center gap-2"><Waves size={20} className="text-aqua-400"/> Pond Intelligence</h3>
                            <button className="text-xs text-aqua-400 hover:text-white underline">View All Ponds</button>
                        </div>
                        ...
                        {getActivePond() && (
                            <div className="bg-aqua-900/40 border border-aqua-700 rounded-2xl p-5 md:p-6 animate-in fade-in zoom-in-95 duration-300">
                                
                                {/* 1. Stocking & Growth Header */}
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                                    <div className="col-span-2 flex flex-col justify-between">
                                        <div>
                                            <h4 className="text-lg font-bold text-white mb-0.5">{getActivePond()?.name} Overview</h4>
                                            <p className="text-aqua-400 text-xs mb-4">Species: {getActivePond()?.stocking.species}</p>
                                        </div>
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                             <div className="bg-aqua-900/80 p-3 rounded-xl border border-aqua-800">
                                                <p className="text-[10px] text-gray-500 uppercase font-bold">DOC</p>
                                                <p className="text-xl font-bold text-white">{getActivePond()?.stocking.doc} <span className="text-[10px] font-normal text-gray-400">days</span></p>
                                            </div>
                                            <div className="bg-aqua-900/80 p-3 rounded-xl border border-aqua-800">
                                                <p className="text-[10px] text-gray-500 uppercase font-bold">Stocking</p>
                                                <p className="text-xl font-bold text-white">{(getActivePond()?.stocking.quantity || 0) / 1000}k</p>
                                            ...
                                            <div className="bg-aqua-900/80 p-3 rounded-xl border border-aqua-800">
                                                ...
                                                <p className="text-xl font-bold text-white">{getActivePond()?.stocking.currentABW} <span className="text-[10px] font-normal text-gray-400">g</span></p>
                                            </div>
                                             <div className="bg-aqua-900/80 p-3 rounded-xl border border-aqua-800">
                                                <p className="text-[10px] text-gray-500 uppercase font-bold">Survival</p>
                                                <p className="text-xl font-bold text-green-400">{getActivePond()?.stocking.survivalRateEst}%</p>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    {/* Feed Chart Mini */}
                                    <div className="bg-aqua-900/60 rounded-xl p-3 border border-aqua-800">
                                        <div className="flex justify-between items-center mb-2">
                                            <h5 className="text-xs font-bold text-gray-300">Feeding (Last 5 Days)</h5>
                                            <span className="text-[10px] bg-aqua-500/20 text-aqua-400 px-1.5 py-0.5 rounded">FCR: {getActivePond()?.feed.fcr}</span>
                                        </div>
                                        <div className="h-28">
                                        ...
                                        <div className="flex justify-between text-[10px] text-gray-400 mt-2 border-t border-aqua-800 pt-1">
                                ...
                                <h5 className="text-xs font-bold text-gray-400 uppercase mb-3 flex items-center gap-2"><Activity size={14}/> Water Parameters</h5>
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
                                    <div className="bg-aqua-900 p-3 rounded-xl border border-aqua-800 text-center">
                                        <p className="text-[10px] text-gray-500 mb-1">pH Level</p>
                                        <p className="text-lg font-bold text-white">{getActivePond()?.water.ph}</p>
                                    ...
                                    <div className="bg-aqua-900 p-3 rounded-xl border border-aqua-800 text-center">
                                        ...
                                        <p className={`text-lg font-bold ${(getActivePond()?.water.do || 0) < 5 ? 'text-red-400' : 'text-white'}`}>{getActivePond()?.water.do} <span className="text-[10px]">mg/L</span></p>
                                    ...
                                    <div className="bg-aqua-900 p-3 rounded-xl border border-aqua-800 text-center">
                                        ...
                                        <p className="text-lg font-bold text-white">{getActivePond()?.water.temp}°C</p>
                                    </div>
                                    <div className="bg-aqua-900 p-3 rounded-xl border border-aqua-800 text-center">
                                        <p className="text-[10px] text-gray-500 mb-1">Salinity</p>
                                        <p className="text-lg font-bold text-white">{getActivePond()?.water.salinity} <span className="text-[10px]">ppt</span></p>
                                    </div>
                                    <div className="bg-aqua-900 p-3 rounded-xl border border-aqua-800 text-center">
                                        <p className="text-[10px] text-gray-500 mb-1">Ammonia</p>
                                        <p className="text-lg font-bold text-white">{getActivePond()?.water.ammonia}</p>
                                    </div>
                                    <div className="bg-aqua-900 p-3 rounded-xl border border-aqua-800 text-center">
                                        <p className="text-[10px] text-gray-500 mb-1">Turbidity</p>
                                        <p className="text-lg font-bold text-white">{getActivePond()?.water.turbidity} <span className="text-[10px]">NTU</span></p>
                                    </div>
                                </div>

                                {/* 3. Medicine & Health Log */}
                                <div className="bg-aqua-800/30 rounded-xl p-3 border border-aqua-800">
                                    <div className="flex items-center gap-2 mb-2">
                                        <Pill size={14} className="text-aqua-400"/>
                                        <h5 className="text-xs font-bold text-white">Recent Health & Medicine Log</h5>
                                    </div>
                                    {getActivePond()?.health && getActivePond()!.health.length > 0 ? (
                                        <div className="flex flex-col gap-2">
                                            {getActivePond()?.health.map((log, idx) => (
                                                <div key={idx} className="flex justify-between items-center bg-aqua-900/50 p-2 rounded-lg text-xs">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[10px] text-gray-400 bg-aqua-800 px-1.5 py-0.5 rounded">{log.date}</span>
                                                        <span className="font-medium text-white">{log.name} ({log.quantity})</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[10px] text-gray-400">{log.reason}</span>
                                                        <span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded-full">{log.type}</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div className="bg-aqua-800/40 border border-aqua-700 p-4 rounded-xl">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-lg font-bold text-white">Geospatial Farm View</h3>
                        </div>
                        <div className="relative w-full h-[400px] bg-aqua-900 rounded-lg overflow-hidden border border-aqua-700 shadow-inner group">
