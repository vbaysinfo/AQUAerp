...
const FinancialAnalytics: React.FC<FinancialAnalyticsProps> = ({ category = 'General' }) => {
  ...
  return (
    <div className="space-y-4 max-w-[1920px] mx-auto pb-6">
       ...
       <div className="flex flex-col md:flex-row justify-between items-end gap-4">
           <div>
                <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                    {category === 'General' ? <BarChart size={24} className="text-aqua-400"/> : 
                     category === 'Feed' ? <Layers size={24} className="text-aqua-400"/> :
                     category === 'Seed' ? <Sprout size={24} className="text-aqua-400"/> :
                     <Pill size={24} className="text-aqua-400"/>}
                    {category} Financial Analytics
                </h2>
                <p className="text-aqua-400 text-xs mt-1">Profitability, cost analysis, and product performance for {category}.</p>
           </div>
           
           {/* AI Prediction Widget */}
           <div className="bg-gradient-to-r from-purple-900/80 to-indigo-900/80 border border-purple-500/50 p-3 rounded-xl shadow-lg max-w-md w-full md:w-auto animate-in fade-in slide-in-from-right-4">
               <div className="flex items-start gap-3">
                   <div className="bg-purple-500/20 p-2 rounded-lg shrink-0">
                       <Sparkles size={18} className="text-purple-400 animate-pulse"/>
                   </div>
                   <div>
                       <p className="text-[10px] font-bold text-purple-300 uppercase mb-1">AI Financial Forecast</p>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
           <div className="bg-aqua-800/50 border border-aqua-700 p-4 rounded-xl">
               <div className="flex justify-between items-start mb-2">
                   ...
                   <div className="bg-green-500/20 p-1 rounded text-green-400"><DollarSign size={14}/></div>
               </div>
               <p className="text-2xl font-bold text-white">$1,280,450</p>
           ...
           <div className="bg-aqua-800/50 border border-aqua-700 p-4 rounded-xl">
               <div className="flex justify-between items-start mb-2">
                   ...
                   <div className="bg-blue-500/20 p-1 rounded text-blue-400"><DollarSign size={14}/></div>
               </div>
               <p className="text-2xl font-bold text-white">$345,120</p>
               <p className="text-[10px] text-green-500 flex items-center gap-1 mt-1"><TrendingUp size={12}/> +8.1% margin</p>
           </div>
           <div className="bg-aqua-800/50 border border-aqua-700 p-4 rounded-xl">
               <div className="flex justify-between items-start mb-2">
                   <p className="text-gray-400 text-xs font-bold uppercase">Stock Value</p>
                   <div className="bg-yellow-500/20 p-1 rounded text-yellow-400"><Package size={14}/></div>
               </div>
               <p className="text-2xl font-bold text-white">$512,000</p>
               <p className="text-[10px] text-gray-400 mt-1">Current inventory assets</p>
           </div>
           <div className="bg-aqua-800/50 border border-aqua-700 p-4 rounded-xl">
               <div className="flex justify-between items-start mb-2">
                   <p className="text-gray-400 text-xs font-bold uppercase">COGS</p>
                   <div className="bg-red-500/20 p-1 rounded text-red-400"><TrendingDown size={14}/></div>
               </div>
               <p className="text-2xl font-bold text-white">$850,200</p>
               <p className="text-[10px] text-red-400 flex items-center gap-1 mt-1"><AlertTriangle size={12}/> +2% cost increase</p>
           </div>
       </div>

       <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
           {/* Main Revenue Trend */}
           <div className="lg:col-span-2 bg-aqua-800/50 border border-aqua-700 p-4 rounded-xl">
               <h3 className="font-bold text-white mb-4 text-sm flex items-center gap-2">
                   <TrendingUp size={16} className="text-aqua-400"/> Revenue vs Profit Trend
               </h3>
           ...
           <div className="bg-aqua-800/50 border border-aqua-700 p-4 rounded-xl">
               <h3 className="font-bold text-white mb-2 text-sm">Revenue Breakdown</h3>
               <div className="h-64">
                   <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie 
               ...
               <div className="mt-2 p-3 bg-aqua-900/50 rounded-lg border border-aqua-800">
                   <p className="text-[10px] text-gray-400 mb-1">Insight</p>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
           {/* Best Performers */}
           <div className="bg-aqua-800/50 border border-aqua-700 rounded-xl p-4 flex flex-col">
               <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                   <ThumbsUp size={16} className="text-green-400" /> Top Performing Products
               </h3>
               <div className="space-y-3 flex-1">
                   {products.top.map((p, i) => (
                       <div key={i} className="bg-aqua-900/50 p-3 rounded-lg border border-aqua-800 flex justify-between items-center">
                           <div>
                               <p className="font-bold text-white text-xs">{p.name}</p>
                               <p className="text-[10px] text-gray-400">Margin: <span className="text-green-400">{p.margin}</span></p>
                           </div>
                           <div className="text-right">
                               <p className="font-bold text-white text-sm">{p.revenue}</p>
           ...
           <div className="bg-aqua-800/50 border border-aqua-700 rounded-xl p-4 flex flex-col">
               <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                   <ThumbsDown size={16} className="text-red-400" /> Areas for Improvement
               </h3>
               <div className="space-y-3 flex-1">
                   {products.bad.map((p, i) => (
                       <div key={i} className="bg-aqua-900/50 p-3 rounded-lg border border-aqua-800 flex justify-between items-center">
                           <div>
                               <p className="font-bold text-white text-xs">{p.name}</p>
                               <p className="text-[10px] text-red-400">{p.reason}</p>
                           </div>
                           <div className="text-right">
                               <p className="font-bold text-white text-sm">{p.revenue}</p>
           ...
           <div className="bg-aqua-800/50 border border-aqua-700 rounded-xl p-4 flex flex-col">
               <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
                   <Users size={16} className="text-blue-400" /> Farmer Usage & Adoption
               </h3>
               <div className="space-y-3 flex-1">
                   {products.popular.map((name, i) => (
                       <div key={i} className="flex items-center justify-between py-2 border-b border-aqua-800/50 last:border-0">
                           <p className="text-xs text-gray-200">{name}</p>
                           <div className="flex items-center gap-2">
                               <div className="w-16 bg-aqua-900 h-1.5 rounded-full">
                                   <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${80 - (i * 15)}%` }}></div>
                               </div>
                               <span className="text-[10px] text-gray-400">{(80 - (i * 15))}% users</span>
                           </div>
                       </div>
                   ))}
               </div>
               <div className="mt-3 p-2 bg-blue-900/20 rounded text-[10px] text-blue-200 text-center">
       ...
       <div className="bg-aqua-800/50 border border-aqua-700 rounded-xl p-4">
           <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-white text-sm">Detailed Profit & Loss Statement ({category})</h3>
                <button className="text-xs text-aqua-400 hover:text-white flex items-center gap-1">
           ...
           <div className="overflow-x-auto">
                <table className="w-full text-xs text-left">
                    ...
                    <tbody className="text-gray-300 divide-y divide-aqua-800/50">
                        <tr>
                            <td className="px-4 py-2.5 font-medium text-white">Revenue from Operations</td>
                            <td className="px-4 py-2.5 text-right text-white font-bold">$1,250,000</td>
                            <td className="px-4 py-2.5 text-right">100.0%</td>
                            <td className="px-4 py-2.5 text-right text-green-400">+5.2%</td>
                        </tr>
                        <tr>
                            <td className="px-4 py-2.5">Cost of Goods Sold (COGS)</td>
                            <td className="px-4 py-2.5 text-right text-red-300">($750,000)</td>
                            <td className="px-4 py-2.5 text-right">60.0%</td>
                            <td className="px-4 py-2.5 text-right text-red-400">+1.2%</td>
                        </tr>
                        <tr className="bg-aqua-900/30">
                            <td className="px-4 py-2.5 font-bold text-aqua-100">Gross Profit</td>
                            <td className="px-4 py-2.5 text-right font-bold text-white">$500,000</td>
                            <td className="px-4 py-2.5 text-right font-medium">40.0%</td>
                            <td className="px-4 py-2.5 text-right text-green-400">+8.5%</td>
                        </tr>
                         <tr>
                            <td className="px-4 py-2.5">Logistics & Distribution</td>
                            <td className="px-4 py-2.5 text-right text-red-300">($80,000)</td>
                            <td className="px-4 py-2.5 text-right">6.4%</td>
                            <td className="px-4 py-2.5 text-right text-green-400">-2.0%</td>
                        </tr>
                        <tr>
                            <td className="px-4 py-2.5">Sales & Marketing</td>
                            <td className="px-4 py-2.5 text-right text-red-300">($45,000)</td>
                            <td className="px-4 py-2.5 text-right">3.6%</td>
                            <td className="px-4 py-2.5 text-right text-gray-500">0.0%</td>
                        </tr>
                         <tr className="bg-aqua-900/50 border-t border-aqua-600">
                            <td className="px-4 py-3 font-bold text-green-400">Net Profit</td>
                            <td className="px-4 py-3 text-right font-bold text-green-400 text-sm">$375,000</td>
                            <td className="px-4 py-3 text-right font-bold">30.0%</td>
                            <td className="px-4 py-3 text-right text-green-400">+12.1%</td>
                        </tr>
                    </tbody>
                </table>
           </div>
